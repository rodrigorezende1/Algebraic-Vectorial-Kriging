...
module Kriging
    ...
    """
    function cormatrix(Xₜ::Matrix{Float64},θ::Vector{Float64})
        ψₘₐₜ = zeros(size(Xₜ, 1), size(Xₜ, 1))
        @inbounds for i in 1:size(Xₜ, 1)
            for j in i+1:size(Xₜ, 1) #Constructing only the upper half
                ψₘₐₜ[i, j] = exp(-sum((1 ./θ).*(-abs.(Xₜ[i, :]-Xₜ[j, :])).^2)) #Function abs() is really important here
    ...
    """
    function vectcor(xₑ::Vector{Float64}, Xₜ::Matrix{Float64},θ::Vector{Float64}) #Here xₑ is every element of Xₑ. So this calculation is done for every evaluation point
        ψₑᵥₐₗ = ones(size(Xₜ, 1),1)
        @inbounds for i in 1:size(Xₜ, 1)
            ψₑᵥₐₗ[i,1] = exp(-sum((1 ./θ).*(-abs.(xₑ.-Xₜ[i, :])).^2)) #The .- is necessary since Xₜ[i, :]::Array{Float64,1}
        end
        return ψₑᵥₐₗ
    end
    ...
    function mle(hyperpar::Union{Union{Array{<:Number, 2}, Array{<:Number, 1}}, Number})
        ...
        θ = 10 .^hyperpar[1:end] #Searching in a log scale
        Nₖᵣ = size(Xₜᵍ, 1)
        Nₒᵤₜ = size(Yₜᵍ,2)
        One = ones(Nₖᵣ, 1)
        ψₘₐₜ = cormatrix(Xₜᵍ, θ)
        if isposdef(ψₘₐₜ)
            U = cholesky(ψₘₐₜ).U
