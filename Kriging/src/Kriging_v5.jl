...
Update 1: MLE_v1, genetic algorithm, Nelder Mead and performance - 25/01/2020
"""
...
module Kriging_v5
    ...
    using Optim #MLE_v1 Nelder Mead
    using BlackBoxOptim #MLE_v1 Genetic
    using SpecialFunctions #Infill Criteria
    export kriging_v1, cormatrix_v1, vectcor_v1, mle_v1, hyperpargenetic_v1, hyperparNelderMead_v1, hyperparmix_v1, PredMSEgenetic_v1, PredMSENelderMead_v1, ExpImpgenetic_v1, ExpImpNelderMead_v1
    ...
    kriging(xₑ, Xₜ, Yₜ) calls the cormatrix_v1(Xₜ) function to construct the
    correlation matrix of the Xₜ data and the vectcor_v1(xₑ, Xₜ) to form the vector
    of correlations between the evaluated point xₑ and the Xₜ.

    """
    function kriging_v1(xₑ::Vector{Float64}, Xₜ::Matrix{Float64}, Yₜ::Matrix{Float64},θ::Vector{Float64},p::Vector{Float64},reg::Float64) #Xₜ is the base training set and Yₜ is the associated fine model response of it; Xₑ is the scenario to be evaluated
          One = ones(size(Xₜ,1),1)
          ψₘₐₜ = cormatrix_v1(Xₜ,θ,p,reg)
          U = cholesky(ψₘₐₜ).U
          μₒₚₜ = ((One)'*(U\(U'\Yₜ)))./((One)'*(U\(U'\One))) #The pointwise operation here is due to the vector output possibility
          ψₑᵥₐₗ = vectcor_v1(xₑ, Xₜ,θ,p, reg) # Vector of correlations between the point Xₑ and the base training Xₜ
          Yₑᵥₐₗ = (μₒₚₜ+ψₑᵥₐₗ'*(U\(U'\(Yₜ-One*μₒₚₜ))))'[:,:]
          return Yₑᵥₐₗ
    end

    """
        cormatrix_v1(Xₜ::Matrix{Float64},θ::Vector{Float64},p::Vector{Float64})
    ...
    The cormatrix_v1(Xₜ::) function implements the construction of the correlation
    ...
    """
    function cormatrix_v1(Xₜ::Matrix{Float64},θ::Vector{Float64},p::Vector{Float64},reg::Float64)
        ...
        ψₘₐₜ = ψₘₐₜ + ψₘₐₜ'+I+I*reg#Constructing ψₘₐₜ and adding a small number to reduce ill conditioning
        return ψₘₐₜ
    end

    """
        vectcor_v1(xₑ::Union{Matrix{Float64},Number}, Xₜ::Matrix{Float64},θ::Union{Matrix{Float64},Number},p::Number)
    ...
    and Xₜ the vectcor_v1(xₑ, Xₜ) uses the correlation function  ψ(x₁, x₂) that
    uses a exponential relationship between the points x₁ and x₂.
    """
    function vectcor_v1(xₑ::Vector{Float64}, Xₜ::Matrix{Float64},θ::Vector{Float64},p::Vector{Float64}, reg::Float64) #Here xₑ is every element of Xₑ. So this calculation is done for every evaluation point
        ψₑᵥₐₗ = ones(size(Xₜ, 1),1)
        @inbounds for i in 1:size(Xₜ, 1)
            ψₑᵥₐₗ[i,1] = exp(-sum(θ.*(abs.(xₑ.-Xₜ[i, :])).^p)) #The .- is necessary since Xₜ[i, :]::Array{Float64,1}
        end
        Ident = zeros(size(ψₑᵥₐₗ,1))
        Ident[end] = reg
        return ψₑᵥₐₗ+Ident # should I put the regression term here as well???
    end

    """
        mle_v1(hyperpar::Union{Union{Matrix{Float64},Array{<:Number,1}},Number})
    ...
    """
    function mle_v1(hyperpar::Union{Union{Array{<:Number, 2}, Array{<:Number, 1}}, Number})
        hyperpar = hyperpar[:]
        #I will have to change here
        p = hyperpar[Int((size(hyperpar,1)+1)/2):end-1][:] #Searching [0.5999,2.0] the range must have an uppper limit of 2.0 otherwise the result can be completely wrong depending on the number of the sampling data
        θ = 10 .^hyperpar[1:Int((size(hyperpar,1)-1)/2)][:] #Searching in a log scale
        reg = 10 .^hyperpar[end]
        Nₖᵣ = size(Xₜᵍ, 1)
        Nₒᵤₜ = size(Yₜᵍ,2)
        One = ones(Nₖᵣ, 1)
        ψₘₐₜ = cormatrix_v1(Xₜᵍ, θ, p, reg)
        if isposdef(ψₘₐₜ)
            U = cholesky(ψₘₐₜ).U
    ...
    set by an genetic algorithm. The MLE_v1 is calculated with the function mle_v1().
    ...
    """
    function hyperpargenetic_v1(Xₜ::Matrix{Float64}, Yₜ::Matrix{Float64})
            global Xₜᵍ, Yₜᵍ
            Xₜᵍ = Xₜ
            Yₜᵍ = Yₜ
            Nᵥ = size(Xₜᵍ, 2)
            res = bboptimize(mle_v1; SearchRange = (-5.0, 2.0), NumDimensions = (2*Nᵥ+1),   Method = :adaptive_de_rand_1_bin_radiuslimited, MaxTime = 60, PopulationSize = 50) #BlackBox Optimization (using BlackBoxOptim)! #SearchRange its really important! #Searching [0.5999,2.0]
            hyperpar = best_candidate(res)
            hyperpar = reshape(hyperpar, size(hyperpar,1),1)
            θ = 10 .^hyperpar[1:Int((size(hyperpar,1)-1)/2)][:]
            p = hyperpar[Int((size(hyperpar,1)+1)/2):end-1][:]
            reg = 10 .^hyperpar[end]
            return θ, p, reg
    end
    ...
    set by the Nelder Mead algorithm. The MLE_v1 is calculated with the function mle_v1().
    ...
    """
    function hyperparNelderMead_v1(Xₜ::Matrix{Float64}, Yₜ::Matrix{Float64})
            global Xₜᵍ, Yₜᵍ
            Xₜᵍ = Xₜ
            Yₜᵍ = Yₜ
            Nᵥ = size(Xₜᵍ, 2)
            lower = ones((2*Nᵥ+1),1)*-5.0
            upper = ones((2*Nᵥ+1),1)*2.0
            initial_x = ones((2*Nᵥ+1),1)*-1
            result = optimize(mle_v1, lower, upper, initial_x, Fminbox(NelderMead()), Optim.Options(show_trace = true, time_limit = 60)) #time_limit = 10 seconds, Fminbox is necesseray otherwise it does not respect the upper and lower bounds
    ...
    the Maximum Likelihood Estimation of Xₜ and Yₜ using values of "θ" and "p"
    set by an mixed genetic and Nelder Mead algorithm. The MLE_v1 is calculated
    with the function mle_v1().
    ...
    The genetic algorithm used is the one offered by the package Optim.jl
    """
    function hyperparmix_v1(Xₜ::Matrix{Float64}, Yₜ::Matrix{Float64})
            global Xₜᵍ, Yₜᵍ
            Xₜᵍ = Xₜ
            Yₜᵍ = Yₜ
            Nᵥ = size(Xₜᵍ, 2)
            res = bboptimize(mle_v1; SearchRange = (-5.0, 2.0), NumDimensions = (2*Nᵥ+1),   Method = :adaptive_de_rand_1_bin_radiuslimited, MaxTime = 500, PopulationSize = 50) #BlackBox Optimization (using BlackBoxOptim)! #SearchRange its really important! #Searching [0.5999,2.0]
            hyperpargen = best_candidate(res)
            hyperpargen = reshape(hyperpargen, size(hyperpargen,1),1)
            #lower = ones((2*Nᵥ+1),1)*minimum(hyperpargen)*1.01
            #upper = ones((2*Nᵥ+1),1)*maximum(hyperpargen)*1.01
            lower = hyperpargen-0.1*abs.(hyperpargen)
            upper =  hyperpargen+0.1*abs.(hyperpargen)
            #result = optimize(mle_v1, lower, upper, hyperpargen, Fminbox(NelderMead()), Optim.Options(show_trace = true, time_limit = 300.0)) #time_limit = 10 seconds, Fminbox is necesseray otherwise it does not respect the upper and lower bounds
            result = optimize(mle_v1, lower, upper, hyperpargen, Fminbox(NelderMead()), Optim.Options(show_trace = true, time_limit = 100))
            hyperpar = Optim.minimizer(result)
            hyperpar = reshape(hyperpar, size(hyperpar,1),1)
            θ = 10 .^hyperpar[1:Int((size(hyperpar,1)-1)/2)][:]
            p = hyperpar[Int((size(hyperpar,1)+1)/2):end-1][:]./5 .+1.599
            reg = 10 .^hyperpar[end]
            return θ, p, reg
    end
...
###################### Infill Criteria ############################
    ...
    function PredMSE_v1(xₑ::Union{Union{Array{<:Number, 2}, Array{<:Number, 1}}, Number})
        ...
        if issubset(xₑ,Xₜ_comp) #Points nearby up to 4 digits will be penalized
            ...
            Nₖᵣ = size(Yₜᵍ,1)
            Nₒᵤₜ = size(Yₜᵍ,2)
            ψₑᵥₐₗ = vectcor_v1(xₑ, Xₜᵍ,θᵍ,pᵍ) # Vector of correlations between the point Xₑ and the base training Xₜ
    ...
    function PredMSEgenetic_v1(Xₜ::Matrix{Float64}, Yₜ::Matrix{Float64},θ::Union{Union{Matrix{Float64},Array{<:Number,1}},Number},p::Number)
        ...
        res = bboptimize(PredMSE; SearchRange = (0.0, 1.0), NumDimensions = (Nᵥ),   Method = :adaptive_de_rand_1_bin_radiuslimited, MaxTime = 600, PopulationSize = 50)
        xₑ_best = best_candidate(res)
        xₑ_best = reshape(xₑ_best, 1,length(xₑ_best))
        return xₑ_best
    end

    function PredMSENelderMead_v1(Xₜ::Matrix{Float64}, Yₜ::Matrix{Float64},θ::Union{Union{Matrix{Float64},Array{<:Number,1}},Number},p::Number)
        global Xₜᵍ, Yₜᵍ,ψₘₐₜᵍ, Uᵍ, μₒₚₜᵍ,θᵍ,pᵍ
        Xₜᵍ = Xₜ
        Yₜᵍ = Yₜ
        θᵍ = θ
        ...
        start_time = time()
        result = optimize(PredMSE, lower, upper, initial_x, Fminbox(NelderMead()), Optim.Options(show_trace = true, time_limit = 600)) #time_limit = 100 seconds, Fminbox is necesseray otherwise it does not respect the upper and lower bounds
    ...
    function ExpImp_v1(xₑ::Union{Union{Array{<:Number, 2}, Array{<:Number, 1}}, Number})
        xₑ =  reshape(xₑ, 1,length(xₑ))
        xₑ = round.(xₑ, digits=3)
        Xₜ_comp = round.(Xₜᵍ, digits=3)
    ...
    function ExpImpgenetic_v1(Xₜ::Matrix{Float64}, Yₜ::Matrix{Float64},θ::Union{Union{Matrix{Float64},Array{<:Number,1}},Number},p::Number)
        global Xₜᵍ, Yₜᵍ,ψₘₐₜᵍ, Uᵍ, μₒₚₜᵍ,θᵍ,pᵍ
        Xₜᵍ = Xₜ
        Yₜᵍ = Yₜ
        θᵍ = θ
        ...
        Uᵍ = U
        μₒₚₜᵍ = ((One)'*(U\(U'\Yₜ)))./((One)'*(U\(U'\One))) #The pointwise operation here is due to the vector output possibility
        res = bboptimize(ExpImp; SearchRange = (0.0, 1.0), NumDimensions = (Nᵥ),   Method = :adaptive_de_rand_1_bin_radiuslimited, MaxTime = 600, PopulationSize = 50)
        xₑ_best = best_candidate(res)
        xₑ_best = reshape(xₑ_best, 1,length(xₑ_best))
        return xₑ_best
    end

    function ExpImpNelderMead_v1(Xₜ::Matrix{Float64}, Yₜ::Matrix{Float64},θ::Union{Union{Matrix{Float64},Array{<:Number,1}},Number},p::Number)
        global Xₜᵍ, Yₜᵍ,ψₘₐₜᵍ, Uᵍ, μₒₚₜᵍ,θᵍ,pᵍ
        Xₜᵍ = Xₜ
        Yₜᵍ = Yₜ
        θᵍ = θ
        ...
        initial_x = ones(Nᵥ,1)*0.5
        start_time = time()
        result = optimize(ExpImp, lower, upper, initial_x, Fminbox(NelderMead()), Optim.Options(show_trace = true, time_limit = 600)) #time_limit = 100 seconds, Fminbox is necesseray otherwise it does not respect the upper and lower bounds
        xₑ_best = Optim.minimizer(result)
        xₑ_best = reshape(xₑ_best, 1,length(xₑ_best))
        return xₑ_best
    end
