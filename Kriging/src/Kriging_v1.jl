...
module Kriging_v1
    ...
    function mle_v1(hyperpar::Union{Union{Array{<:Number, 2}, Array{<:Number, 1}}, Number})
        ...
        p = hyperpar[Int(size(hyperpar,1)/2)+1:end][:]./5 .+1.599 #Searching [0.5999,2.0] the range must have an uppper limit of 2.0 otherwise the result can be completely wrong depending on the number of the sampling data
    ...
    function hyperpargenetic_v1(Xₜ::Matrix{Float64}, Yₜ::Matrix{Float64})
            ...
            Yₜᵍ = Yₜ
            Nᵥ = size(Xₜᵍ, 2)
            res = bboptimize(mle_v1; SearchRange = (-3.0, 2.0), NumDimensions = (2*Nᵥ),   Method = :adaptive_de_rand_1_bin_radiuslimited, MaxTime = 300.0, PopulationSize = 50) #BlackBox Optimization (using BlackBoxOptim)! #SearchRange its really important! #Searching [0.5999,2.0]
    ...
    function hyperparNelderMead_v1(Xₜ::Matrix{Float64}, Yₜ::Matrix{Float64})
            ...
            Yₜᵍ = Yₜ
            Nᵥ = size(Xₜᵍ, 2)
            lower = ones((2*Nᵥ),1)*-3.0
            upper = ones((2*Nᵥ),1)*2.0
            initial_x = ones((2*Nᵥ),1)*-1
            result = optimize(mle_v1, lower, upper, initial_x, Fminbox(NelderMead()), Optim.Options(show_trace = true, time_limit = 300.0)) #time_limit = 10 seconds, Fminbox is necesseray otherwise it does not respect the upper and lower bounds
