...
module Kriging_v4
    ...
    export kriging_v1, cormatrix_v1_v1, vectcor_v1_v1, mle_v1, hyperpargenetic_v1, hyperparNelderMead_v1, hyperparmix_v1, PredMSEgenetic_v1, PredMSENelderMead_v1, ExpImpgenetic_v1, ExpImpNelderMead_v1
    ...
    """
    function kriging_v1(xₑ::Vector{Float64}, Xₜ::Matrix{Float64}, Yₜ::Matrix{Float64},θ::Vector{Float64},p::Vector{Float64}) #Xₜ is the base training set and Yₜ is the associated fine model response of it; Xₑ is the scenario to be evaluated
          One = ones(size(Xₜ,1),1)
          ψₘₐₜ = cormatrix_v1(Xₜ,θ,p)
          U = cholesky(ψₘₐₜ).U
          μₒₚₜ = ((One)'*(U\(U'\Yₜ)))./((One)'*(U\(U'\One))) #The pointwise operation here is due to the vector output possibility
          ψₑᵥₐₗ = vectcor_v1(xₑ, Xₜ,θ,p) # Vector of correlations between the point Xₑ and the base training Xₜ
    ...
    """
    function cormatrix_v1(Xₜ::Matrix{Float64},θ::Vector{Float64},p::Vector{Float64})
    ...
    """
    function vectcor_v1(xₑ::Vector{Float64}, Xₜ::Matrix{Float64},θ::Vector{Float64},p::Vector{Float64}) #Here xₑ is every element of Xₑ. So this calculation is done for every evaluation point
    ...
    function mle_v1(hyperpar::Union{Union{Array{<:Number, 2}, Array{<:Number, 1}}, Number})
        ...
        p = hyperpar[Int(size(hyperpar,1)/2)+1:end][:]./5 .+1.5999 #Searching [0.5999,2.0] the range must have an uppper limit of 2.0 otherwise the result can be completely wrong depending on the number of the sampling data
        θ = 10 .^hyperpar[1:Int(size(hyperpar,1)/2)][:] #Searching in a log scale
        Nₖᵣ = size(Xₜᵍ, 1)
        Nₒᵤₜ = size(Yₜᵍ,2)
        One = ones(Nₖᵣ, 1)
        ψₘₐₜ = cormatrix_v1(Xₜᵍ, θ, p)
        if isposdef(ψₘₐₜ)
            U = cholesky(ψₘₐₜ).U
    ...
    function hyperpargenetic_v1(Xₜ::Matrix{Float64}, Yₜ::Matrix{Float64})
            ...
            Yₜᵍ = Yₜ
            Nᵥ = size(Xₜᵍ, 2)
            res = bboptimize(mle_v1; SearchRange = (-5.0, 2.0), NumDimensions = (2*Nᵥ),   Method = :adaptive_de_rand_1_bin_radiuslimited, MaxTime = 600.0, PopulationSize = 50) #BlackBox Optimization (using BlackBoxOptim)! #SearchRange its really important! #Searching [0.5999,2.0]
            hyperpar = best_candidate(res)
            hyperpar = reshape(hyperpar, size(hyperpar,1),1)
            θ = 10 .^hyperpar[1:Int(size(hyperpar,1)/2)][:]
            p = hyperpar[Int(size(hyperpar,1)/2)+1:end][:]./5 .+1.5999
            return θ, p
    end
    ...
    function hyperparNelderMead_v1(Xₜ::Matrix{Float64}, Yₜ::Matrix{Float64})
            ...
            Yₜᵍ = Yₜ
            Nᵥ = size(Xₜᵍ, 2)
            lower = ones((2*Nᵥ),1)*-5.0
            upper = ones((2*Nᵥ),1)*2.0
            initial_x = ones((2*Nᵥ),1)*-1
            result = optimize(mle_v1, lower, upper, initial_x, Fminbox(NelderMead()), Optim.Options(show_trace = true, time_limit = 600.0)) #time_limit = 10 seconds, Fminbox is necesseray otherwise it does not respect the upper and lower bounds
    ...
    function hyperparmix_v1(Xₜ::Matrix{Float64}, Yₜ::Matrix{Float64})
            ...
            Yₜᵍ = Yₜ
            Nᵥ = size(Xₜᵍ, 2)
            res = bboptimize(mle_v1; SearchRange = (-5.0, 2.0), NumDimensions = (2*Nᵥ),   Method = :adaptive_de_rand_1_bin_radiuslimited, MaxTime = 450.0, PopulationSize = 50) #BlackBox Optimization (using BlackBoxOptim)! #SearchRange its really important! #Searching [0.5999,2.0]
            hyperpargen = best_candidate(res)
            hyperpargen = reshape(hyperpargen, size(hyperpargen,1),1)
            #lower = ones((2*Nᵥ),1)*minimum(hyperpargen)*1.01
            #upper = ones((2*Nᵥ),1)*maximum(hyperpargen)*1.01
            lower = hyperpargen-0.5*abs.(hyperpargen)
            upper =  hyperpargen+0.5*abs.(hyperpargen)
            ...
            result = optimize(mle_v1, lower, upper, hyperpargen, Fminbox(NelderMead()), Optim.Options(show_trace = true, time_limit = 150.0))
            hyperpar = Optim.minimizer(result)
            hyperpar = reshape(hyperpar, size(hyperpar,1),1)
            θ = 10 .^hyperpar[1:Int(size(hyperpar,1)/2)][:]
            p = hyperpar[Int(size(hyperpar,1)/2)+1:end][:]./5 .+1.599
            return θ, p
    end
...
###################### Infill Criteria ############################
    ...
    function PredMSEgenetic_v1(Xₜ::Matrix{Float64}, Yₜ::Matrix{Float64},θ::Union{Union{Matrix{Float64},Array{<:Number,1}},Number},p::Number)
        ...
        res = bboptimize(PredMSE; SearchRange = (0.0, 1.0), NumDimensions = (Nᵥ),   Method = :adaptive_de_rand_1_bin_radiuslimited, MaxTime = 600.0, PopulationSize = 50)
    ...
    function PredMSENelderMead_v1(Xₜ::Matrix{Float64}, Yₜ::Matrix{Float64},θ::Union{Union{Matrix{Float64},Array{<:Number,1}},Number},p::Number)
        ...
        start_time = time()
        result = optimize(PredMSE, lower, upper, initial_x, Fminbox(NelderMead()), Optim.Options(show_trace = true, time_limit = 600.0)) #time_limit = 100 seconds, Fminbox is necesseray otherwise it does not respect the upper and lower bounds
    ...
    function ExpImpgenetic_v1(Xₜ::Matrix{Float64}, Yₜ::Matrix{Float64},θ::Union{Union{Matrix{Float64},Array{<:Number,1}},Number},p::Number)
        ...
        Uᵍ = U
        μₒₚₜᵍ = ((One)'*(U\(U'\Yₜ)))./((One)'*(U\(U'\One))) #The pointwise operation here is due to the vector output possibility
        res = bboptimize(ExpImp; SearchRange = (0.0, 1.0), NumDimensions = (Nᵥ),   Method = :adaptive_de_rand_1_bin_radiuslimited, MaxTime = 600.0, PopulationSize = 50)
        xₑ_best = best_candidate(res)
        xₑ_best = reshape(xₑ_best, 1,length(xₑ_best))
        return xₑ_best
    end
    ...
    function ExpImpNelderMead_v1(Xₜ::Matrix{Float64}, Yₜ::Matrix{Float64},θ::Union{Union{Matrix{Float64},Array{<:Number,1}},Number},p::Number)
        ...
        initial_x = ones(Nᵥ,1)*0.5
        start_time = time()
        result = optimize(ExpImp, lower, upper, initial_x, Fminbox(NelderMead()), Optim.Options(show_trace = true, time_limit = 600.0)) #time_limit = 100 seconds, Fminbox is necesseray otherwise it does not respect the upper and lower bounds
        xₑ_best = Optim.minimizer(result)
        xₑ_best = reshape(xₑ_best, 1,length(xₑ_best))
        return xₑ_best
    end
